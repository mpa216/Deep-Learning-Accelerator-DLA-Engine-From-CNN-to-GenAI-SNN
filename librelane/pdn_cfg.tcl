...
# APIC_A CHANGE: the 3 gf180 OCD SRAM macros (C buffer) carry their
# VDD/VSS power pins on Metal2/Metal3 (top power layer = Metal3), NOT on
# the Metal4/Metal5 strap layers. The stock connect "Metal4 Metal5" makes
# no vias to the macro pins, so the macro grid ends up with no shapes/vias
# -> [PDN-0232] -> [PDN-0233] failed to generate full power grid.
# Connect the SRAM Metal3 power pins UP to the Metal4 vertical straps of
# the stdcell grid that pass over each macro.
add_pdn_connect \
    -grid macro \
