...
module dla_engine_top_g3005_tb;
    localparam int N = 1;
    localparam int K = 256;
    localparam int ROWS = 784;
    localparam int ROW_IDX = 5;
    ...
    reg signed [DATA_W-1:0] weights_all [0:(ROWS*K)-1];
    reg signed [DATA_W-1:0] bias_all [0:ROWS-1];
    ...
    initial begin
        ...
        if (!$value$plusargs("INPUT_MEMH=%s", input_memh)) begin
            input_memh = "tb/data/g3005_input.memh";
        end
        if (!$value$plusargs("EXP_MEMH=%s", expected_memh)) begin
            expected_memh = "tb/data/g3005_expected.memh";
        end
        if (!$value$plusargs("VCD=%s", vcd_path)) begin
            vcd_path = "dla_engine_top_g3005_tb.vcd";
        end
        if (!$value$plusargs("ACTUAL_MEMH=%s", actual_memh)) begin
            actual_memh = "tb/data/g3005_actual.memh";
        end
        if (!$value$plusargs("ACTUAL_TXT=%s", actual_txt)) begin
            actual_txt = "tb/data/g3005_actual.txt";
        end

        $readmemh({weights_dir, "/G300_4_weight.memh"}, weights_all);
        $readmemh({weights_dir, "/G300_4_bias.memh"}, bias_all);
        $readmemh(input_memh, input_vec);
        $readmemh(expected_memh, expected_val);

        $dumpfile(vcd_path);
        $dumpvars(0, dla_engine_top_g3005_tb);
    end
    ...
    initial begin
        ...
        // Load selected G300_4 row weights into A buffer.
        for (i = 0; i < K; i = i + 1) begin
            @(posedge clk);
            wr_en <= 1'b1;
            wr_sel <= 1'b0;
            wr_addr <= i[AB_ADDR_W-1:0];
            wr_data <= weights_all[(ROW_IDX*K) + i];
        end
        @(posedge clk);
        wr_en <= 1'b0;
        ...
        begin
            ...
            bias_ext = {{(ACC_W - DATA_W){bias_all[ROW_IDX][DATA_W-1]}}, bias_all[ROW_IDX]};
