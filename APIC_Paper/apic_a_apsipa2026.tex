...
FP16 is a different datapath rather than a parameter change---exponent addition, alignment shifting, normalization and rounding around an $11\times11$ mantissa multiplier---and the open PDK provides no floating-point macro. Its arithmetic core alone is therefore bounded below by the INT12 row. The dynamic range that motivates floating point is already handled here by the per-tensor scale together with the Q4.12 requantization, and the measured saturation counters confirm that the integer path is not clipping where it matters.
