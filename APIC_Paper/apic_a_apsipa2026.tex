...
That compute time is not, however, the latency a user sees. Streaming the weights across the 4-wire link costs one 20-bit frame per byte. The bridge shifts on rising \texttt{SCLK} edges only, behind a two-flop synchronizer that needs each level stable for about three core clocks, so a transferred bit costs a full serial period of six core clocks---240~ns at the 40~ns core clock, the rate the pad-level testbench drives and passes at. The 331{,}776 weight bytes of one image therefore take 1.59~s. Adding the input vectors, the \texttt{START} commands and the C read-back gives \textbf{1.612~s end to end per image}, of which the PE array is busy for 0.22\%: the link is $446\times$ the compute. Effective throughput is 0.62 images/s, or 0.41~MOPS against the 0.8~GOPS peak.

This is a property of the workload, not an implementation defect. A matrix--\emph{vector} product reuses no weight, so 1.01~weight bytes must cross the link per MAC and no achievable on-chip SRAM changes that; with 538~KiB of model against 2.8~KiB of buffer, the host is necessarily inside the inner loop. The consequences are worth stating plainly for anyone reproducing this class of design: peak GOPS is close to meaningless here, and the useful levers are all on the data-movement side. Widening the host interface is the largest: an 8-bit parallel write bus reduces the cost from 20 serial bit-periods per byte to one, and batching several latents against one weight stream amortizes the dominant term directly. Both were implemented and measured on a follow-on variant of this design, which reaches $62\times$ end-to-end over its own baseline serial link by combining the two. Weight-stationary reuse, double buffering, or a compressed weight format would attack the same term.
